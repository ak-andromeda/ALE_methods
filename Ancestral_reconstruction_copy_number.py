...
def make_node_data(df, cutoff):
    ...
    for node in nodes:
        ...
        df_node_cut = df_node[df_node["Copies"] >= 0.50]
...
def main():
    df = run_node_reconstruction()
    df_total = make_node_data(df, cutoff)
    print(df_total)
